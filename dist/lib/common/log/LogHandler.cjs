...
function canDoAction(actionMode, allowedLevels, level) {
    return actionMode === common_log_contracts.ActionMode.Always
        || actionMode !== common_log_contracts.ActionMode.Never && (allowedLevels & level) !== 0;
}
class LogHandler {
    constructor({ name, logger, allowLogLevels, maxLogSize, }) {
        ...
        this.allowLogLevels = allowLogLevels == null
            ? common_log_contracts.LogLevel.Any
            : allowLogLevels;
    ...
    canLog(logEvent) {
        return !this.disabled && canDoAction(logEvent.handlersModes
            ? logEvent.handlersModes[this.name] || logEvent.handlersModes._all || common_log_contracts.ActionMode.Default
            : common_log_contracts.ActionMode.Default, this.allowLogLevels, logEvent.level);
    }
    onError(logEvents, error) {
        handleLogErrorHandler(logEvents, error, this._logger, newLogEvent => {
            ...
            newLogEvent.handlersModes[this.name] = common_log_contracts.ActionMode.Never;
        });
    }
    enqueueLog(logEvent) {
